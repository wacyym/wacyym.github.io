...
/* Canvas 1.0 by YellowAfterlife */


function window_get_width() {
	return typeof(window.innerWidth) == 'number' ? window.innerWidth : document.documentElement && document.documentElement.clientWidth ? document.documentElement.clientWidth : document.body && document.body.clientWidth ? document.body.clientWidth : 720;
}
function window_get_height() {
	return typeof(window.innerHeight) == 'number' ? window.innerHeight : document.documentElement && document.documentElement.clientHeight ? document.documentElement.clientHeight : document.body && document.body.clientHeight ? document.body.clientHeight : 540;
}

function canvas_set_position(x, y) {
	tu_canvas.style.position = 'absolute';
	tu_canvas.style.left = x + 'px';
	tu_canvas.style.top = y + 'px';
}

function canvas_set_style(key, value) {
	tu_canvas.style[key] = value;
}
function canvas_get_style(key) {
	return tu_canvas.style[key];
}
function canvas_create(width, height) {
	var r = document.createElement('canvas');
	r.width = width;
	r.height = height;
	r.context = r.getContext('2d');
	return r;
}
 ...
 ***********************************************************************/
function __font() { 
__font_init__(this, font, 'Times', 14, 1, 0)}; var font = new __font();
...
with(this) {
x = mouse_x - lightSize/2;
y = mouse_y - lightSize/2;
...
__handle_sprite__(this);
with(this) {
//draw_text(mouse_x,mouse_y,global.time);

var radialGradient = tu_context.createRadialGradient(x + lightSize / 2,y + lightSize / 2,0,x + lightSize / 2, y + lightSize / 2, lightSize / 2);

radialGradient.addColorStop(0, "rgba(246, 236, 226, 1)");
radialGradient.addColorStop(1, "rgba(210, 200, 190, 1)");
tu_context.globalCompositeOperation = "overlay";
...
if (dis_t > len)
{       
   this.xx[0]+= lengthdir_x(dis_t-len,dir_t);
   this.yy[0]+= lengthdir_y(dis_t-len,dir_t);
}                           


for(i=1;i<=this.sg;i++)
{     
    dis_t= point_distance(this.xx[i],this.yy[i],this.xx[i-1],this.yy[i-1]);
    dir_t = point_direction(this.xx[i],this.yy[i],this.xx[i-1],this.yy[i-1]);

    if (dis_t > len)
    {
       this.xx[i]+= lengthdir_x(dis_t-len,dir_t);
       this.yy[i]+= lengthdir_y(dis_t-len,dir_t);
    }
}                  
// For tail
}
...
with(this) {
this.other = this.place_meeting(this.x, this.y, obj_spermatozoon);
if(this.other != null) {
if (id != other)
{
...
}; var obj_spermatozoon = new __obj_spermatozoon();
...
var scene_12 = new __scene_12();
tu_scenes.push(scene_12);
function __scene_12_dup() { 
this.tiles = [
];
this.objects = [
...
var scene_12_dup = new __scene_12_dup();
tu_scenes.push(scene_12_dup);
tu_room_to_go = scene_12;
